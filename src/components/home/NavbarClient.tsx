...
const NavbarClient = ({ dataUser }: IProps) => {
    ...
    return (
        <header
            ...
            className={`sticky top-0 left-0 w-full transition-all duration-500 z-200 ${showBg ? `bg-black border-b border-white/60 top-0 container ${isOpen ? "md:rounded-br-full rounded-br-2xl" : "md:rounded-b-full rounded-b-2xl"} z-100000` : ""}
            ${isOpen ? "bg-black border-b border-white/60 top-0" : ""}
            `}>
            {/* overlay */}
            <div onClick={() => setIsOpen(false)} className={isOpen ? "fixed inset-0 bg-black/50" : "hidden"} />
            ...
            >
                ...
                <ul className={`${isOpen ? "w-fit md:md:w-125 flex flex-col gap-4 absolute top-20 left-0 p-5 bg-black/95 h-screen" : "hidden lg:flex lg:items-center gap-3"}`}>
                    {
                ...
                <div>
                    {
                        dataUser
                            ? (
                                <Link href={"/profile"} className="bg-transparent border-b border-r border-l cursor-pointer hover:bg-white hover:text-black duration-300 border-white text-white py-2 px-4 rounded-b-md">
                                    Profile
                                </Link>
                            )
                            : (
