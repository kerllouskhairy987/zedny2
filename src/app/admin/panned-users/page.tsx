...
import { GetPannedUsers } from "@/server/admin/users/action";
import { Pen } from "lucide-react";
import Link from "next/link";


const page = async () => {
    const pannedUsers = await GetPannedUsers();
        ...
        <section className="p-5 ps-10">
            <div>
                <Table className="overflow-x-auto ">
                    ...
                    <TableBody>
                        {
                            pannedUsers && pannedUsers.length === 0
                                ...
                                : pannedUsers.map((pan) => {
                                    const created = getDateInfo(pan.createdAt);
                                    const updated = getDateInfo(pan.updatedAt);

                                    return (
                                        <TableRow key={pan.id}>
                                            <TableCell className="border text-center font-medium">{pan.id}</TableCell>
                                            ...
                                            <TableCell className="border text-center font-medium min-w-50 max-w-125 whitespace-normal wrap-break-word">
                                                <span className={`${pan.isAdmin ? "bg-[#358BAD]" : "bg-pink-500"} px-3 py-1 w-2 h-2 rounded-full inline-block}`}>
                                                    {pan.isAdmin ? "Admin" : "User"}
                                                </span>
                                            </TableCell>
                                            <TableCell className="border text-center font-medium min-w-50 max-w-125 whitespace-normal wrap-break-word">
                                                <span className={`${pan.pan ? "bg-red-500" : "bg-green-500"} px-3 py-1 w-2 h-2 rounded-full inline-block}`}>
                                                    {pan.pan ? "Panned" : "Not Panned"}
                                                </span>
                                            </TableCell>
                                            <TableCell className="text-center font-semibold flex justify-center gap-3">
                                                <DeleteUserAccount id={pan.id} />

                                                <Link href={`/admin/users/update/${pan.id}`}>
                    ...
                    <TableFooter>
                        <TableRow>
                            ...
                            <TableCell className="text-end">({pannedUsers.length}) user/s</TableCell>
                        </TableRow>
                    </TableFooter>
                </Table>
            </div>
